...
r_VOLUME, g_VOLUME, b_VOLUME = hex2rgb(HTML_color_VOLUME)
...
function draw_volume(cr,x_pos,y_pos,start_rect_height,r_color_VOLUME,g_color_VOLUME,b_color_VOLUME,transparency_VOLUME,indicator)
-- Draw indicator
	cairo_set_source_rgba(cr,r_color_VOLUME,g_color_VOLUME,b_color_VOLUME,transparency_VOLUME)
	cairo_set_operator(cr, operator_transpose[mode])
	cairo_set_line_width(cr, 3)
	...
	cairo_move_to(cr,x_pos+10+34-15-align,(start_rect_height/2)+y_pos)
	cairo_rel_line_to(cr,0,-h)
	cairo_rel_line_to(cr,0,h*2)
	cairo_rel_line_to(cr,w,0)
	cairo_rel_line_to(cr,w,h2)
	cairo_rel_line_to(cr,0,-(h2*2+h*2))
	cairo_rel_line_to(cr,-w,h2)
	cairo_rel_line_to(cr,-w,0)
	cairo_fill(cr)

	if tonumber(indicator) < 25 then
		cairo_set_line_width(cr, 1)
	end
	cairo_arc(cr,x_pos+10+34-align+8,(start_rect_height/2)+y_pos,10,300*math.pi/180,(420)*math.pi/180)
	cairo_stroke(cr)	
	if tonumber(indicator) < 50 then
		cairo_set_line_width(cr, 1)
	end
	cairo_arc(cr,x_pos+10+34-align+8,(start_rect_height/2)+y_pos,20,300*math.pi/180,(420)*math.pi/180)
	cairo_stroke(cr)
	if tonumber(indicator) < 75 then
		cairo_set_line_width(cr, 1)
	end		
	cairo_arc(cr,x_pos+10+34-align+8,(start_rect_height/2)+y_pos,30,300*math.pi/180,(420)*math.pi/180)
	cairo_stroke(cr)
...
function draw_function(cr)
	...
	if battery then
		...
		vol = conky_parse('${exec amixer get Master | grep "Front Left:" | awk \'{print $5}\' | tr -d []%%}')
		draw_circle(cr,(start_rect_width_no_battery)+gap_x_fix,gap_y_fix,r_VOLUME, g_VOLUME, b_VOLUME, transparency_VOLUME,vol)
		draw_volume(cr,(start_rect_width_no_battery)+gap_x_fix,gap_y_fix,start_rect_height,r_VOLUME,g_VOLUME,b_VOLUME,transparency_VOLUME,vol)		
