...
ffffff,000000,f1f2f2,424141,323131,313131
...
while [ 1 ];do
	_has_error=0
	if [ -z ${newcolor} ]; then 
			read -p "Please enter your new background color in #RRGGBB(The '#' is a must!, #db7b4d is currently set): " newcolor
	fi
...
#some fail colors(colors already used by theme)
	case "$(echo ${newcolor}|sed 's/#//'|tr '[:upper:]' '[:lower:]')" in
		BB2835|285CBB|BB6F28|288ABB|5DBB28|ffffff|000000|f1f2f2|424141|323131|313131)
			printf "\nSorry the colors: BB2835,285CBB,BB6F28,288ABB,5DBB28,ffffff,000000,f1f2f2,424141,323131,313131 are not available!\n"
			_has_error=1;;
	esac
...
#recolor the theme
find "$basedir" -type f -not -path "*/.git/*" -not -path "gtksourceview" -not -path "metacity-1" -not -path "xfwm4" -exec sed -i 's/#db7b4d/'$newcolor'/g' {} \;
find "$basedir" -type f -not -path "*/.git/*" -not -path "gtksourceview" -not -path "metacity-1" -not -path "xfwm4" -exec sed -i 's/#dB7B4D/'$newcolor'/g' {} \;
...
find "$basedir" -type f -not -path "*/.git/*" -not -path "gtksourceview" -not -path "metacity-1" -not -path "xfwm4" -exec sed -i 's/#F2F1F1/'$newcolor2' {} \;
cat <<\EOF
